...
from .segments import DEFAULT_MAX_PARTS, SEGMENT_MODES, SEGMENT_MODE_INHERIT
...
class Settings:
    ...
    outbox_segmented_reply: str = SEGMENT_MODE_INHERIT
    """How a proactive message is cut into bubbles.

    ``inherit`` follows the host's own 分段回复 switch, ``on`` segments even when
    the host has it off, ``off`` delivers one bubble. A proactive message does not
    pass through the host's ``result_decorate``/``respond`` stages, so without this
    the Runtime's "没说完就再发一条" arrived as a single bubble with an embedded
    newline while her replies to the same person arrived as several.
    """

    outbox_segment_max_parts: int = DEFAULT_MAX_PARTS
    """Ceiling on bubbles per proactive message; the tail is merged, never cut."""
    ...
    def from_mapping(cls, raw: Mapping[str, Any] | None) -> Settings:
        ...
        segment_mode = (
            as_str(data.get("outbox_segmented_reply"), SEGMENT_MODE_INHERIT).strip().lower()
        )
        if segment_mode not in SEGMENT_MODES:
            issues.append(
                f"outbox_segmented_reply must be one of {', '.join(SEGMENT_MODES)} "
                f"(got {segment_mode!r}); falling back to {SEGMENT_MODE_INHERIT}",
            )
            segment_mode = SEGMENT_MODE_INHERIT
        ...
        return cls(
            ...
            outbox_segmented_reply=segment_mode,
            outbox_segment_max_parts=count(
                "outbox_segment_max_parts",
                DEFAULT_MAX_PARTS,
                0,
                16,
            ),
