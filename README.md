...
| `input_debounce_ms` | `0` | 输入防抖窗口（毫秒），`0` 为关闭。见 §3「input_debounce_ms」 |
| `input_debounce_max_chars` | `4000` | 防抖合并的文本上限，超出时从最早的一条开始丢 |
...
| `outbox_segmented_reply` | `inherit` | 主动消息分段：`inherit` 跟宿主「分段回复」开关走 / `on` 强制分 / `off` 整条发。见 §3「为什么主动消息也要分段」 |
| `outbox_segment_max_parts` | `4` | 主动消息最多分几条（`0` 不限）；超出的并进最后一条，不丢字 |
...
### input_debounce_ms：连发多条只回一次

默认 `0`（关闭）。设为正数（建议 2000–3000）后，同一个会话的 LLM 请求会在 `on_llm_request`
上先等一个窗口：窗口内用户继续发消息，则**只有最后一条**真的触发回复，前面几条的正文被合并进
这一轮的 prompt（换行拼接，超过 `input_debounce_max_chars` 时从最早的一条开始丢）。
用户「在吗」「睡了没」「？」三连，得到一条回复，而不是三条。

三个必须说清楚的点：

1. **为什么要合并文本，而不是简单丢掉前面的请求。** AstrBot 把「用户消息 + 助手回复」作为**一对**
   在 LLM 回答之后才写进会话历史（`internal.py` 的 `update_conversation`）。所以在 `on_llm_request`
   里被 `stop_event()` 停掉的那一轮**永远进不了历史** —— 只取消不合并，那几句话就从模型的视野里
   永久消失了（Runtime 侧仍然有原始事件，两边会不一致）。合并是唯一不会丢字的做法；也正因为被停掉的
   轮次没进历史，合并不会让模型看到重复的同一句话。
2. **原始事件不会丢。** 观察发生在**收到消息时**（`on_message_observed`），不是在这里。连发三条就是
   三条 raw event，Runtime 的认知输入不因防抖而变粗；变的只是"触发几次 LLM"。
3. **不接管"要不要说话"。** 防抖只影响**用户发消息后**这一轮的合并；Runtime 的主动消息走 outbox，
   完全不过这里。它也不是"冷却"，不会让角色因此少说话。

已知代价：窗口内回复会晚 `input_debounce_ms` 才发出；用户若在这期间发了图片，图片不会跟着合并
（合并的是 `req.prompt` 的文本）。

### outbox_segmented_reply：为什么主动消息也要分段

宿主的回复**是被切开发出去的**：`result_decorate` 按 `platform_settings.segmented_reply`
把每条 `Plain` 切成若干条，`respond` 再一条一条发、每条之间停一下（默认 0.8–1.6s 随机）。
主动消息走的是 `Context.send_message`，**不经过这两个 stage** —— 于是 Runtime 渲染成两行
的话，在 QQ 里变成**一条**带换行的气泡，而同一个人收到的她的回复是两条。

这不是格式问题：Runtime 的渲染风格里写着「我说话就一两句……**没说完就再发一条**，别堆成
一大段」，也就是说"分几条说"是写在**写作侧**的意图，却在**投递侧**丢了。实测旧库里 4 条
已发出的主动消息里有 3 条带换行，全部当一条发出去。

于是插件在投递时按**宿主自己的规则**切分：

- **规则来自宿主配置**（`platform_settings.segmented_reply`：`split_mode`/`regex`/
  `split_words`/`words_count_threshold`/`interval`/`interval_method`），每次发送时现读，
  所以 WebUI 里改开关或间隔立刻生效，不需要重载插件。默认 `inherit`：**宿主的「分段回复」
  开关同时管两边**，不产生第二个权威；`on` 强制分段，`off` 回到整条发。
- **只切分，不改写**。宿主自己的路径还会对每条应用 `content_cleanup_rule`（`[。]+$`，去掉
  尾句号）；这里**不做** —— 文本是 Runtime 写的，适配器的契约是把它送出去，不是改它。
  每条只做 `strip()`，那是切分本身必需的。
- **绝不丢字**。超过 `outbox_segment_max_parts`（默认 4）时，多出来的部分**并进最后一条**
  而不是被截断；超过宿主的 `words_count_threshold`（默认 60 字）则整条发，跟宿主一致。
- **每段之间按宿主的间隔停顿**（`send_timeout_ms` 以内；租约在发送期间照常续租）。
- **回报按实际发出的条数**。第一条就失败 → 照旧报失败让 Runtime 重发；后面的失败 →
  `sent=true` + `delivered_segments=n`，因为前面几条已经在用户那儿了，把整条报成失败
  只会让用户第二次看到同一句。写回宿主历史的也只有**真的发出去的那些**。
...
- `0.1.0`（未发版）：**主动消息分段**（`outbox_segmented_reply` / `outbox_segment_max_parts`）。
  宿主的分段回复只作用于走 pipeline 的 LLM 结果，主动消息走 `Context.send_message` 绕开了
  `result_decorate`/`respond`，于是 Runtime 渲染成两行的话到 QQ 里是一条带换行的气泡。现在
